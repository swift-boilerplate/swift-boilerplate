...
private let scheduler = QueueScheduler(qos: .background, name: "com.facestar.jpapi", targeting: nil)
                ...
                .flatMap(.concat) { data, response -> SignalProducer<Data, ErrorEnvelope> in
                    ...
                    guard
                        ...
                        let contentType = headers["Content-Type"], contentType.hasPrefix("application/json")
                            ...
                            print("[JPAPI] Failure \(self.sanitized(request))")
                    ...
                    print("[JPAPI] Success \(self.sanitized(request))")
...
private let defaultSessionError =
    NSError(domain: "com.facestar.JpApi.rac_dataWithRequest", code: 1, userInfo: nil)
